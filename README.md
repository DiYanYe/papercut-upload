# 这是一个适用于东方理工校园网络打印服务的文件上传自动化工具

1. **功能介绍：** 将要打印的文件自动上传到papercut网页。把`papercut-upload.cmd`文件下载后即可使用。
...
3. **使用方法**
    ...
    **法1：** 直接把要打印的文件拖到`papercut-upload-delay.cmd` 上（可选中多文件后一起拖入），输入打印机编号，按回车后会自动把要打印的文件上传到papercut网页。若直接输入回车，会选择默认打印机打印。
    
    **法2：** 在命令行运行（可多文件）：papercut-upload-delay.cmd "C:\path\a.pdf" "C:\path\b.docx"
